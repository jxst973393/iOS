...
class xqTableViewController: UITableViewController,UINavigationControllerDelegate,UIGestureRecognizerDelegate  {
    ...
    let alert = UIAlertController(title: "请选择", message: nil, preferredStyle: .actionSheet)
        ...
        let action6 = UIAlertAction(title: "取消收藏", style: .default) { (UIAlertAction) in
            self.scshanchu()
        }
...
//
            xqCloud["plusername"] = self.crea["Name"] as? String
////
            
//            if let imgFile = self.crea["images"] as? AVFile{
//                self.xqimage.image = UIImage(data: imgFile.getData()!)
//            }
            
            xqCloud["images"] = self.crea["images"] as? AVFile
...
//            qqq.setObject(testt, forKey: "usersc")
            ...
            xqCloud.setObject(user, forKey: "uss")
...
//        let quer = AVQuery(className: "sc")
//
//        quer.findObjectsInBackground(){ (results,error) in
//            if let results = results as? [AVObject]{
//                self.creas = results
//
//                for i in self.creas {
//                    let iii = i["plgl"] as? String
//                    print(iii,"iii")
//                    if  iii == self.currentUser?.objectId {
...
//                }
//
////                let ccc = self.creas["plgl"] as? String
//
////                if let ccc = self.currentUser?.objectId {
////                    alert.addAction(actiont)
////                    alert.addAction(action6)
////                    //            alert.addAction(action5)
////                    alert.addAction(action3)
...
//            print(results,"error")
//
////            if results["plgl"] as! String == currentUser?.objectId {
////                alert.addAction(actiont)
////                alert.addAction(action6)
////                //            alert.addAction(action5)
////                alert.addAction(action3)
...
//
//
//        let query = AVObject(className: "sc")
//        query.fetchInBackground() { (results,error) in
//
//
//            let ccc = self.currentUser?.objectId
////            let ttt = results!["plgl"] as? String
////            print(ttt,"ttt")
//
////            if ccc == ttt {
////                alert.addAction(actiont)
...
//
//        }
//        let ccc = currentUser?.objectId
//        let ttt = query["plgl"] as? String
//        print(query,"query")
//        print(ccc,"ccc")
//        print(ttt,"ttt")
//        if query["plgl"] as?String == currentUser?.objectId  {
//            alert.addAction(actiont)
//            alert.addAction(action6)
////            alert.addAction(action5)
//            alert.addAction(action3)
...
//                alert.addAction(action)
//                alert.addAction(cancel)
//        let query = AVObject(className: "cs", objectId: self.crea.objectId!)
//
//        let ccc = AVObject(className: "sc")
//
//        let  ttt = query["scsc"] as? String
//
//        if   ttt == ccc.objectId{
//            alert.addAction(actiont)
//            alert.addAction(action6)
...
//        }
        ...
        let ccc = AVObject(className: "cs", objectId: self.crea.objectId!)
        let ttttt = AVQuery(className: "sc")
        ttttt.findObjectsInBackground(){ (results,error) in
            if let results = results as? [AVObject] {
                self.creas = results
            
//                for i in self.creas{
//                    print(i["usersc"] ,"string")
//                    let qqq = i["usersc"] as? String
//                    let www = ccc["objectId"] as? String
//                    print(www!,"www")
//
//                    i.fetchInBackground(withKeys: ["usersc"], block: { (results, error) in
//                        let ttt = results?.object(forKey: "usersc") as? AVObject
//
//                        if ttt?.objectId == ccc.objectId{
//                            alert.addAction(action6)
//                            alert.addAction(action3)
//                            alert.addAction(action4)
//                            alert.addAction(action)
...
//                for i in self.creas{
//
//                    print(i.objectId,"666")
//                    let iii = i["usersc"] as? String
//                    print(ccc.objectId,"ccc.objectid")
//
//                    if  iii == ccc.objectId {
//                        alert.addAction(action6)
//                        alert.addAction(action3)
...
//
//                for i in self.creas {
//                    let test = AVObject(className: "sc", objectId: i.objectId!)
//
...
//
//                        let ccc = ttt!["plgl"] as? String
//
//                        if ccc == self.currentUser?.objectId {
//                                        alert.addAction(actiont)
//                                        alert.addAction(action6)
//                                        alert.addAction(action3)
//                                        alert.addAction(action4)
...
//        let objec = AVObject(className: "sc")
//        print(objec,"objec")
...
//    @IBAction func scuser(segue : UIStoryboardSegue){
//        if segue.identifier == "scuser"{
//            //获取返回的控制器
//            let  shouc = segue.source as! scTableViewController
//
////            shouc.shoucang = scuser
////            print(scuser.objectId,"scuser")
//
//        }
//    }
...
//        let test = AVObject(className: "sc", objectId: crea.objectId!)
//
//        test.fetchInBackground(withKeys: ["plgl"], block: {(results,error) in
//            let ttt = results?.object(forKey: "plgl") as? AVObject
//
